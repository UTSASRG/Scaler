...
eSystemPhase g_phase;
eSystemAction g_action;
bool g_hasRollbacked;
bool g_bugReproduced;
int g_rbNumber;
#ifdef TESTSEGV 
int g_rbInSegvNumber;
unsigned long g_rbInSegvStartTime;
unsigned long g_rbInSegvEpochStartTime;
unsigned long trace_inst_count;
#endif
pthread_cond_t g_condCommitter;
pthread_cond_t g_condWaiters;
pthread_mutex_t g_mutex;
pthread_spinlock_t g_spinlock; //Hongyu
pthread_mutex_t g_mutexSignalhandler;
int g_waiters;
// Maintain the total number of threads
int g_alivethreads;
int g_waitersTotal;
int waitingFenceThreads;
bool waitingFence;
#ifdef GET_CHARECTERISTICS
unsigned long count_epochs = 0;
unsigned long count_actual_epochs = 0;
unsigned long count_syscalls = 0;
unsigned long count_syncevent = 0;
#endif
...
__thread int cache_fd;
__thread socketCachedDataInfo* cache_socket_data;
...
__attribute__((constructor)) void initializer() {
  ...
  if(!initialized) {
	...
	//watchpoint::getInstance().addWatchpoint((void *)0x1001a5Ea8, sizeof(void *), OBJECT_TYPE_WATCHONLY, NULL, 0);
	//watchpoint::getInstance().addWatchpoint((void *)0x1001B5EE8, sizeof(void *), OBJECT_TYPE_WATCHONLY, NULL, 0);
...
int doubletake_main(int argc, char** argv, char** envp) {
  /******** Do initialization here (runs after ctors) *********/
	initializer();

	// Register the exit function
	atexit(finalizer);

  // Now we will enter into the normal execution mode.
  global_setNormalExecution();
  ...
  PRINF("Main thread %d at %p has finished now.\n", current->index, current);

  // this is main thread, we will not let it really exit
  if(!normal_exit) {
    // stop here
  }
...
#ifdef DL_SUPPORT
// it seems glibc uses its internal dlopen function,
// we can not intercept dlopen out of application
void *dlopen(const char *filename, int flags) {
  void* ret = NULL;
  if(current->disablecheck){
    ret = Real::dlopen(filename, flags);
  }else{
    ret = syscalls::getInstance().dlopen(filename, flags);
  }
...
extern "C" {
  void* xxmalloc(size_t sz) {
    ...
    //PRINT("MEMORY allocation %zu at %p\n", sz, ptr);
    //fprintf(stderr, "MEMORY allocation %zu at %p\n", sz, ptr);
    return ptr;
  }

  void xxfree(void* ptr) {
    if(initialized && ptr) {
      //fprintf(stderr, "MEMORY free at %p\n", ptr);
  ...
  void * xxmemalign(size_t alignment, size_t size) {
    if(!initialized) {
      void* ptr = xxmalloc(size + 2*alignment);
      void* alignedPtr = (void *) (((size_t) ptr + alignment - 1) & ~(alignment - 1));
      return alignedPtr;
    }
    return xmemory::getInstance().memalign(alignment, size);
  }
...
extern "C" {
  ...
  int pthread_mutex_trylock(pthread_mutex_t* mutex) {
    // PRINT("inside pthread_mutex_trylock, line %d!\n", __LINE__);
   	if(initialized) {
      if(current->disablecheck) {
  ...
  int pthread_mutex_unlock(pthread_mutex_t* mutex) {
    ...
    if(initialized) {
      if(current->disablecheck){
        ...
        return xthread::getInstance().mutex_unlock(mutex);
      }
    }
  ...
  int pthread_rwlock_init(pthread_rwlock_t *__restrict rwlock, const pthread_rwlockattr_t *__restrict attr) {
    ...
    if(initialized){
      if(current->disablecheck) {
        ...
        return xthread::getInstance().rwlock_init(rwlock, attr);
      }
    }
    return 0;
  }
  ...
  int pthread_rwlock_rdlock(pthread_rwlock_t *rwlock) { 
    if(initialized) {
      if(current->disablecheck){
        return Real::pthread_rwlock_rdlock(rwlock); 
      }else{
        return xthread::getInstance().rwlock_rdlock(rwlock);
      }
    }
  ...
  int pthread_rwlock_wrlock(pthread_rwlock_t *rwlock) {
    if(initialized) {
      if(current->disablecheck){
        ...
        return xthread::getInstance().rwlock_wrlock(rwlock);
      }
    }
  ...
  int pthread_rwlock_unlock(pthread_rwlock_t *rwlock) {
    if(initialized) {
      if(current->disablecheck){
        ...
        return xthread::getInstance().rwlock_unlock(rwlock);
      }
    }
  ...
  int pthread_rwlock_destroy(pthread_rwlock_t *rwlock) {
    return xthread::getInstance().rwlock_destroy(rwlock);
  }
  ...
  int pthread_cond_init(pthread_cond_t* cond, const pthread_condattr_t* condattr) {
    if(initialized){
      if(current->disablecheck) {
        return Real::pthread_cond_init(cond, condattr); 
      } else {
        return xthread::getInstance().cond_init(cond, condattr);
      }
    }
    return 0;
  }
  ...
  */
  ...
  int pthread_create(pthread_t* tid, const pthread_attr_t* attr, void* (*start_routine)(void*),
		     void* arg) {
		//fprintf(stderr, "creation start_routine %p\n", start_routine);
  ...
  int pthread_once(pthread_once_t *once_control, void (*init_routine)(void)){
    ...
    if(!global_isRollback()) {
      ...
      ret = Real::pthread_once(once_control, init_routine);
      if(!isCheckDisabled) {
        xthread::enableCheck();
      }
    }
...
*/
  ...
  */
  ...
  int close(int fd) {
  //fprintf(stderr, "close fd %d ****** in libireplayer\n", fd);
    if(!initialized) {
  ...
  FILE* fopen(const char* filename, const char* modes) {
    //fprintf(stderr, "fopen in libireplayer\n");
    if(!initialized) {
			if(!funcInitialized) {
			  initRealFunctions();
			}
  ...
  FILE* fopen64(const char* filename, const char* modes) {
   ...
   // fprintf(stderr, "fopen64 in libireplayer\n");
    return syscalls::getInstance().fopen64(filename, modes);
  }
  
/* 
  FILE* freopen(const char* path, const char* mode, FILE* stream) {
  	fprintf(stderr, "freopen %d ****** in libireplayer not supported\n", __LINE__);
    abort();
  }
*/
  ...
  FILE *freopen(const char* filename, const char* mode, FILE* stream) {
      ...
      return syscalls::getInstance().freopen(filename, mode, stream);
  }
  ...
  int fflush(FILE *stream){
	return syscalls::getInstance().fflush(stream);
  }
  ...
  */
  ...
  int poll(struct pollfd *fds, nfds_t nfds, int timeout) {
		fprintf(stderr, "poll in doubletake at %d\n", __LINE__);
		return syscalls::getInstance().poll(fds, nfds, timeout);
	}
#else 
	int __poll_chk (struct pollfd *__fds, nfds_t __nfds, int __timeout, __SIZE_TYPE__ __fdslen){
		return syscalls::getInstance().poll(__fds, __nfds, __timeout);
	}
#endif
  ...
  */
  ...
  int mincore(void* start, size_t length, unsigned char* vec) {
    ...
    if(!initialized || current->disablecheck) {
      ...
      PRERR("mincore is not supported!!!");
      ret = -1;
    }
    return ret;
  ...
  */
  ...
  int accept4(int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags){
    return syscalls::getInstance().accept4(sockfd, addr, addrlen, flags);
  }

  ssize_t send(int sockfd, const void *buf, size_t len, int flags){ //Hongyu
	  return syscalls::getInstance().send(sockfd, buf, len, flags);
  }

  ssize_t recv(int sockfd, void *buf, size_t len, int flags){ //Hongyu
	  return syscalls::getInstance().recv(sockfd, buf, len, flags); 
  }
  ...
  //  pid_t fork(){ 
  pid_t fork(void){
    return syscalls::getInstance().fork();
  }
  ...
  */
  ...
  int semctl(int semid, int semnum, int cmd, ...) {
    ...
    */
    ...
    return syscalls::getInstance().semctl(semid, semnum, cmd, arg);
  }
 ...
 // int sigwait(const sigset_t *set, int *sig); //Hongyu
  ...
  */
  ...
  //fprintf(stderr, " in doubletake at %d\n", __LINE__);
    ...
    fprintf(stderr, "prctl is not supported now\n");
    abort();
...
//fprintf(stderr, " in doubletake at %d\n", __LINE__);
  ...
  */
  ...
  int sigwait(const sigset_t *set, int *sig){
	  return syscalls::getInstance().sigwait((sigset_t *)set, sig);
  }
