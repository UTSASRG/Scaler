    ...
    for threadId in recInfo.threadIdList:
        ...
        if len(curThreadRecArray) != len(aggregatedTimeArray):
            ...
            for i, curRec in enumerate(curThreadRecArray[:-1]):
                ...
                if aggregatedTimeArray[i].flags & (1 << 0):
                    # Use mean and count to estimate total clock cycles
                    aggregatedTimeArray[i].totalClockCycles += curRec.count * curRec.totalClockCycles / 1024
                else:
    ...
    for curFileRecord in timingRecord:
        for curExtFileRecord in curFileRecord.extFileTiming.values():
            for curExtSymRecord in curExtFileRecord.extSymTiming.values():
                ...
                if curExtFileRecord.totalClockCycles.value > 0:
            ...
            if curFileRecord.childrenClockCycles.value > 0:
                curExtFileRecord.totalClockCycles.parentPercent = curExtFileRecord.totalClockCycles.value / curFileRecord.childrenClockCycles.value
            else:
