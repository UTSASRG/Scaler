  ...
  inline bool isOverflow(void* addr, void* /* objectStart */, size_t /* objectSize */) { // FIXME
	...
	inline bool checkObjectOverflow(void * ptr, size_t blockSize, size_t sz, bool cleanSentinels) {
      ...
      if(nonAlignedBytes == 0) {
		...
		// Clear sentinels, if the size is equal to blocksize,
		// we do not clean the always exiting magic word.
		if(cleanSentinels && offset!=0) {
			clear(pclean);
		}
        ...
        watchpoint::getInstance().addWatchpoint(p, *((unsigned long*)p), OBJECT_TYPE_OVERFLOW, ptr, sz); //Hongyu
      }
...
private:
  ...
  inline unsigned long getBytes(unsigned long elements) { return (elements * BYTEBITS); }
