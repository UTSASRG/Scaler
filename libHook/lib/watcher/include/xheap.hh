...
#ifdef MEMORY_DUMP
  void saveMemToDisk(int flag){
	  PRINF("save memory files %d\n", flag);
	  std::string filename = std::string("memdata.") + to_string(flag);
	  FILE *fp = fopen(filename.c_str(), "wb+");
	  fwrite((void*)_start, 1, ((intptr_t)_position-(intptr_t)_start), fp);
	  fclose(fp);
  }
#endif
