...
enum fileOpAttr {
	F_OP_READ = 0, // Default one
  F_OP_WRITE,
};

struct fcntlRecordLocks {
  int threadidx;
  int fd;
  int cmd;
  struct flock lock; 
...
class fops {
  ...
  public:
    ...
    fileOpAttr opattr;	
    fileOpAttr currentopattr;	
    
    pthread_mutex_t mutex;
    //TODO Now we don't free this memory, even file is closed
    RecordEntries<struct fcntlRecordLocks> reclist;
  };
...
public:
  ...
  void updateOpenedFiles() {
    ...
    for(i = _filesMap.begin(); i != _filesMap.end(); i++) {
      ...
      thisFile->pos = pos;
      thisFile->opattr = thisFile->currentopattr;
      thisFile->reclist.cleanup();
  ...
  void saveDupFd(int oldfd, int newfd) {
    ...
    _sysrecord.getFileOps(E_SYS_FILE_DUP, &fd);
  ...
  int closeDir(DIR* dir) {
    ...
    _sysrecord.deferDirClose(E_SYS_DIR_CLOSE, dir);
  ...
  void saveFd(int fd, FILE* file) {
    ...
    if(fd != -1) {
      ...
      thisFile->opattr = F_OP_READ;
      thisFile->currentopattr = F_OP_READ;

      // use our library to init it, so that it can be controlled
      pthread_mutex_init(&thisFile->mutex, NULL);
      thisFile->reclist.initialize(xdefines::MAX_FCNTL_RECORD_LOCK_ENTRIES);
  ...
  void updateFInfo(FILE* file, char* buf){
	  fileInfo* finfo;
	  int fd = file->_fileno;
	//	PRINT("update %d origStream %p\n", fd, file);
  ...
  void updateFileAttr(int fd, fileOpAttr attr = F_OP_WRITE){
    fileInfo* finfo;
    if(_filesMap.find(fd, sizeof(fd), &finfo)) {
      finfo->currentopattr = attr;
    }
  }

  bool isFileReadOnly(int fd){
    bool ret = false;
    fileInfo* finfo;
    if(_filesMap.find(fd, sizeof(fd), &finfo)) {
      ret = (finfo->currentopattr == F_OP_READ);
    }
    return ret;
  }
  ...
  void prepareRollback() {
    ...
    for(i = _filesMap.begin(); i != _filesMap.end(); i++) {
      fileInfo* thisFile;
      thisFile = (fileInfo*)i.getData();
      thisFile->currentopattr = thisFile->opattr;
      thisFile->reclist.prepareRollback();
      	...
      	if(thisFile->origStream) {
          // FIXME it should be reset when it is opened. Because the memory may be reused somewhere 
  ...
  FILE* getFopen() {
    ...
    if(fd == -1) {
      ...
      malloc(xdefines::FOPEN_ALLOC_SIZE);
      file = NULL;
    } else {
      fileInfo* finfo;
      if(!_filesMap.find(fd, sizeof(fd), &finfo)) {
      ...
      // check whether the pointer is the same as before.
      if(ptr != finfo->origStream){
        PRINT("thread %d [%d] getFopen fd %d and file %p current ptr %p\n", current->index, current->tid, fd, finfo->origStream, ptr);
        abort();
      }
  ...
  int getFdAtOpen() {
    ...
    _sysrecord.getFileOps(E_SYS_FILE_OPEN, &fd);
  ...
  //==================================
  //we use synchronization functions in our library, 
  //so if there is no space for new event, it can invoke a new epoch and release holding lock
  void saveFcntlRecLock(int threadidx, int fd, int cmd, long arg, int ret, int errcode){
    fileInfo* thisFile;
    if(!_filesMap.find(fd, sizeof(fd), &thisFile)) {
      PRINT("can not find fd %d for thread %d", fd, threadidx);
      assert(0);
    }

    pthread_mutex_lock(&thisFile->mutex);
    fcntlRecordLocks* record = thisFile->reclist.alloc();
    record->threadidx = threadidx;
    record->fd = fd;
    record->cmd = cmd;
    record->ret = ret;
    record->errcode = errcode;
    memcpy(&record->lock, (void*)arg, sizeof(struct flock));
    pthread_mutex_unlock(&thisFile->mutex);
  }

  void getFcntlRecLock(int threadidx, int fd, int cmd, long arg, int* ret, int* errcode){
    fileInfo* thisFile;
    if(!_filesMap.find(fd, sizeof(fd), &thisFile)) {
      PRINT("can not find fd %d for thread %d", fd, threadidx);
      assert(0);
    ...
    pthread_mutex_lock(&thisFile->mutex);
    fcntlRecordLocks* record = thisFile->reclist.retrieveIterEntry();
    
    assert(record->threadidx == threadidx);
    assert(record->fd == fd);
    assert(record->cmd == cmd);

    memcpy((void*)arg, &record->lock, sizeof(struct flock));
    *ret = record->ret;
  ...
  */
  int closeFile(int fd, FILE* fp) {
    ...
    _sysrecord.deferFileClose(E_SYS_FILE_CLOSE, fd);
    return ret;
  }
  ...
  void cleanClosedFile(int fd) {
    ...
    if(_filesMap.find(fd, sizeof(fd), &thisFile)) {
      ...
      if(thisFile->origStream) {
        Real::fclose(thisFile->origStream);
				...
				// Now we will free the buffer related to this origstream.
		  	free(thisFile->buf); //Hongyu
      ...
      InternalHeap::getInstance().free(thisFile);
			...
			PRERR("Abort because of wrong file fd %d\n", fd); 
      // Should not happen.
      assert(0);
    }
	}

	void cleanClosedDir(DIR * dir) {
    ...
    if(_dirsMap.find(dir, sizeof(dir), &thisDir)) {
      ...
      InternalHeap::getInstance().free(thisDir);
			...
			PRERR("Abort because of wrong dir entry %p\n", dir); 
      // Should not happen.
      assert(0);
    }
  }
